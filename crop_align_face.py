...
def CropAlignFace(RawImageDir, AlignedImageDir,
                  LandmarksModelPath='landmarks/shape_predictor_68_face_landmarks.dat'):
    """
    Extracts and aligns all faces from images using DLib and a function from original FFHQ dataset preparation step
    python align_images.py /raw_images /aligned_images

    Optionally saves landmarks for each face with SaveLandmarks set to True and a LandmarksDir specified
    """
    ...
    for img_name in os.listdir(RawImageDir):
        raw_img_path = os.path.join(RawImageDir, img_name)
        
        for i, face_landmarks in enumerate(landmarks_detector.get_landmarks(raw_img_path), start=1):
            face_img_name = '%s_%02d.png' % (os.path.splitext(img_name)[0], i)
            aligned_face_path = os.path.join(AlignedImageDir, face_img_name)
